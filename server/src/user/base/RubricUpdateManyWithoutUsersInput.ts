/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/docs/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { RubricWhereUniqueInput } from "../../rubric/base/RubricWhereUniqueInput";
import { ApiProperty } from "@nestjs/swagger";
@InputType()
class RubricUpdateManyWithoutUsersInput {
  @Field(() => [RubricWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [RubricWhereUniqueInput],
  })
  connect?: Array<RubricWhereUniqueInput>;

  @Field(() => [RubricWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [RubricWhereUniqueInput],
  })
  disconnect?: Array<RubricWhereUniqueInput>;

  @Field(() => [RubricWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [RubricWhereUniqueInput],
  })
  set?: Array<RubricWhereUniqueInput>;
}
export { RubricUpdateManyWithoutUsersInput };
